...
background_image = pygame.image.load("D:/Pygame/The Tower Guardian/ecran accueil.png").convert()
...
class Button:
    def __init__(self, x, y, width, height, text, font, bg_color, text_color, border_color, border_width):
        self.rect = pygame.Rect(x, y, width, height)
        self.text = text
        self.font = font
...
# Classe Monstre
class Monstre:
    def __init__(self, x, y, width, height, speed, color, damage_cooldown, enemy_type):
        self.rect = pygame.Rect(x, y, width, height)
        self.speed = speed
        self.color = color
        ...
        self.enemy_type = enemy_type  # Type d'ennemi : "fire" pour esprit de feu, "slime" pour slime
        ...
        if self.wave_count < self.max_waves:
            if current_time - self.last_spawn_time > self.interval * 1000:
                if self.monsters_spawned < self.max_monsters_per_wave and self.total_monsters_spawned < self.total_monsters_limit:
                    ...
                    if self.current_enemy_type == "fire":
                        self.monsters.append(Monstre(random_platform.rect.x + random_platform.rect.width, random_platform.rect.y - 50, 50, 50, random.randint(1 + self.wave_count, 3 + self.wave_count), self.color, 60, "fire"))
                    else:
                        self.monsters.append(Monstre(random_platform.rect.x + random_platform.rect.width, random_platform.rect.y - 50, 50, 50, random.randint(1 + self.wave_count, 3 + self.wave_count), self.color, 60, "slime"))
                    ...
                    if current_time - self.wave_finished_time > self.time_between_waves:
                        ...
                        self.wave_finished_time = 0
                        self.total_monsters_spawned = 0
                        # Alterner entre les types d'ennemis à chaque vague
                        if self.current_enemy_type == "fire":
                            self.current_enemy_type = "slime"
                        else:
                            self.current_enemy_type = "fire"
...
class Fireball:
    def __init__(self, x, y, direction):
        self.x = x
        self.y = y
        self.width = 115  # Nouvelle largeur du sprite
        self.height = 115  # Nouvelle hauteur du sprite
        self.radius = 20
        self.speed = 10
        self.direction = direction
        self.sprites = []  # Liste pour stocker les sprites de l'animation
        # Charger chaque sprite de l'animation
        for i in range(1, 6):  # Supposons que vous avez 5 images numérotées de 1 à 5
            sprite_path = f"D:/Pygame/The Tower Guardian/Mobs/Boule de feu/Fireball{i}.png"
            sprite = pygame.image.load(sprite_path)
            # Modifier la taille du sprite ici
            sprite = pygame.transform.scale(sprite, (self.width, self.height))  # Modifiez les dimensions selon vos besoins
            self.sprites.append(sprite)
        self.current_sprite_index = 0  # Indice du sprite actuel dans la liste

    def move(self):
        self.x += self.speed * self.direction

    def draw(self, window):
        window.blit(self.sprites[self.current_sprite_index], (self.x - self.width // 2, self.y - self.height // 2))  # Centrer le sprite
...
# Définir les caractéristiques du bouton
font = pygame.font.SysFont("Arial", 48)
play_button = Button(WINDOW_WIDTH // 2 - 100, WINDOW_HEIGHT // 2 + 300, 200, 80, "Play", font, WHITE, BLACK, BLACK, 5)
...
# Fonction pour dessiner uniquement le bouton
def draw_button():
    window.blit(background_image, (-0, 0))
    play_button.draw(window)
    pygame.display.flip()
