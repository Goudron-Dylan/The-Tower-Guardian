...
class Personnage:
    def __init__(self, x, y, sprites_right, sprites_left, attack_sprites_right, attack_sprites_left, speed):
        ...
        self.last_damage_time = 0  # Temps depuis le dernier dégât
        self.damage_cooldown = 1000  # Temps en millisecondes avant de pouvoir subir un nouveau dégât
        self.rect = pygame.Rect(x, y, self.width, self.height)  # Initialisation du rectangle de collision
        self.can_take_damage = True  # Ajouter cette ligne pour initialiser la variable de dégâts
    ...
    def move(self):
        if not self.is_attacking:
            if self.is_moving_right:
                ...
                self.x -= self.speed
        self.rect.topleft = (self.x, self.y)  # Mettre à jour la position du rectangle de collision
    ...
    def check_collision(self):
        ...
        if current_time - self.last_damage_time > self.damage_cooldown:  # Vérifier si le temps écoulé dépasse le cooldown
            for monster in spawner.monsters:
                if (self.x < monster.rect.right and
                    self.x + self.width > monster.rect.left and
                    self.y < monster.rect.bottom and
                    self.y + self.height > monster.rect.top):
                    self.health -= 1
                    self.last_damage_time = current_time  # Mettre à jour le temps du dernier dégât
                    if self.health <= 0:
                        # Gérer la mort du personnage ici...
                        pass
                    break

# Classe Monstre
class Monstre:
    def __init__(self, x, y, width, height, speed, color, health):
        self.rect = pygame.Rect(x, y, width, height)
        self.speed = speed
        self.color = color
        self.health = health  # Ajout de l'attribut health pour représenter la santé du monstre
        ...
        if self.wave_count < self.max_waves:
            if current_time - self.last_spawn_time > self.interval * 1000:  # Convertir l'intervalle en millisecondes
                if self.monsters_spawned < self.max_monsters_per_wave and self.total_monsters_spawned < self.total_monsters_limit:
                    ...
                    self.monsters.append(Monstre(random_platform.rect.x + random_platform.rect.width, random_platform.rect.y - 50, 50, 50, random.randint(1 + self.wave_count, 3 + self.wave_count), self.color, self.wave_count + 1))  # Réduire la santé du monstre en fonction du niveau de la vague
...
player = Personnage(50, WINDOW_HEIGHT // 2, player_sprites_right, player_sprites_left, attack_sprites_right, attack_sprites_left, 5)
    ...
    if player.is_attacking:
        ...
        if player.current_sprite >= len(player.attack_sprites_right):
            ...
            if player.is_moving_right:
                ...
                fireball = Fireball(player.x - 0.1 * player.width, player.y + player.height // 2, -1)  # Ajuster la position de départ de la boule de feu
    ...
    for fireball in fireballs:
        ...
        for monster in spawner.monsters:
            if fireball.x + fireball.radius > monster.rect.left and fireball.x - fireball.radius < monster.rect.right and fireball.y + fireball.radius > monster.rect.top and fireball.y - fireball.radius < monster.rect.bottom:
                # La boule de feu a touché un monstre
                monster.health -= 1  # Réduire la santé du monstre
                if monster.health <= 0:  # Si la santé du monstre atteint 0 ou moins, le supprimer
...
#dgt en fonction du temps et non des ennemis à changer