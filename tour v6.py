...
class Monstre:
    def __init__(self, x, y, width, height, speed, color, damage_cooldown):
        ...
        self.can_attack = False  # Ajouter un attribut pour vérifier si le monstre peut attaquer
    ...
    def attack_tower(self, wave_number):
        # Vérifier si le monstre peut attaquer en fonction de l'intervalle entre les attaques
        current_time = pygame.time.get_ticks()
        if current_time - self.attack_timer > self.attack_interval and self.can_attack:
            # Attaquer la tour en fonction du numéro de la vague
            if wave_number == 1:
                tour.take_damage(1)
            elif wave_number == 2:
                tour.take_damage(2)
                ...
                tour.take_damage(4)
            # Ajoutez d'autres cas pour chaque vague supplémentaire ici
...
tour = Tour(0, 0, 300, WINDOW_HEIGHT, 100)
...
while running:
    ...
    for monster in spawner.monsters:
        monster.attack_tower(spawner.wave_number)  # Passer le numéro de la vague actuelle
    ...
    # Vérifier si la tour est détruite
    if tour.health <= 0:
        print("La tour a été détruite. Vous avez perdu !")
        running = False

    # Vérifier si le joueur est mort
    if player.health <= 0:
        print("Vous avez été vaincu par les ennemis. Vous avez perdu !")
        running = False
