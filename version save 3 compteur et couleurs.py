        ...
        if self.wave_count < self.max_waves:
            if current_time - self.last_spawn_time > self.interval * 1000:  # Convertir l'intervalle en millisecondes
                if self.monsters_spawned < self.max_monsters_per_wave and self.total_monsters_spawned < self.total_monsters_limit:
                    ...
                    self.monsters.append(Monstre(self.rect.x + self.rect.width, self.rect.y, 50, 50, random.randint(1 + self.wave_count, 3 + self.wave_count), self.color))
...
while running:
    ...
    for spawner in spawners:
        spawner.spawn_monster()  # Appel de la méthode spawn_monster() pour chaque spawner
        spawner.draw(window)
    ...
    # Affichage des compteurs de monstres
    total_monsters_spawned = sum(spawner.total_monsters_spawned for spawner in spawners)
    monsters_spawned_text = font.render(f"Monstres générés : {total_monsters_spawned}", True, text_color)
    window.blit(monsters_spawned_text, (WINDOW_WIDTH // 2 - monsters_spawned_text.get_width() // 2, 200))
